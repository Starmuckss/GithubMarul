...
output_directory = dir_path + "\\data\\pre_semt_correlation" # Output Data will be saved in here
...
semtler = pd.read_pickle("new_semtler.pkl")  # Get from Merge Semt Links 
semtler = semtler.drop_duplicates()
...
try:
    if len(sampled_dict_of_pairs) > len(np.load(output_directory + "\\sampled_dict_of_pairs_for_correlation.npy",allow_pickle='TRUE').item()):
        np.save(output_directory + "\\sampled_dict_of_pairs_for_correlation.npy",sampled_dict_of_pairs)         
except FileNotFoundError:
    np.save(output_directory + "\\sampled_dict_of_pairs_for_correlation.npy",sampled_dict_of_pairs)         
