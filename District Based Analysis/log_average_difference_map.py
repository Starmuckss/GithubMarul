...
input_directory = dir_path + "\\data\\semt_log_average_abs_difference" # Preprocessed data will be recorded here
output_directory = dir_path + "\\output\\log_average_difference_maps" # Output Data will be saved in here
...
for category in category_list:
    if os.path.exists(input_directory + "\\" + category + "_semt.pkl"):
    ...
    try:
        ...
        merged.plot(ax=ax,column='log_average_difference', legend=True,cmap = "Blues" ,missing_kwds={
        "color": "gray",
        "label": "Missing values",
    })
        plt.axis('off')

        fig.savefig(output_directory+"\\"+category,dpi=600) # 600
        fig.clf()
