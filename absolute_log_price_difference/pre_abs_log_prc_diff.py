...
Sep/2022: preprocess step for absolute log price difference metric  
@author: HP
"""
import pandas as pd
import os
...
class absolute_log_price_difference_preprocess():
    ...
    def preprocess(self):
        ...
        for category in self.category_list:
            for pair in sampled_dict_of_pairs[category]:                 
                for pos in pair: # pos is pair[0] or pair[1]
                    ...
                    log_average_df_directory = self.output_directory + "\\" + pos_name + "_" + category+ ".pkl"
        ...
        np.save(self.output_directory + "\\sampled_dict_of_pairs.npy",sampled_dict_of_pairs) 
        time.sleep(5) # wait for sample_dict to save 
