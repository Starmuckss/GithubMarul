...
class shared_identical_prices_within_chain():
    ...
    def check_pairs_daily2(self,dataframe,pair):
        """
        ...
        for category in self.category_list:
            ...
            for market in market_dictionary.keys(): 
                ...
                for pair in list_of_pairs:
                        ...
                        if os.path.exists(directory +"\\"+ pair[0] + "\\" + category + ".pkl") and os.path.exists(directory +"\\"+ pair[1] + "\\" + category + ".pkl"): #Check if these pair exists in this date
                            ...
                            if len(pair_prices) != 0:
                                ...
                                if len(iterating_df_of_a_pair) == 0:    
                                    iterating_df_of_a_pair = self.check_pairs_daily2(pair_prices, pair)
                                    iterating_df_of_a_pair["Tot_days"] = 1 
                                else:    
                                    temp_df = self.check_pairs_daily2(pair_prices, pair)
                    ...
                    if len(iterating_df_of_a_pair) > 0:
                        iterating_df_of_a_pair = iterating_df_of_a_pair[iterating_df_of_a_pair['Tot_days'] >= len(self.dates)*0.5] # uncomment to filter days
            ...
            category_df.to_csv(self.output_directory+"//"+category +"_"+"within_chain_identical_prices.csv")
