# -*- coding: utf-8 -*-
"""
Created on Tue Feb 23 11:50:46 2021
...
output_directory = dir_path+"\\Share Of Identical Prices" # Data will be printed out here, You have to create this directory before using it.

if not os.path.exists(output_directory):
    os.mkdir(output_directory)
categories = pd.read_excel("categories.xlsx")
category_list = list(categories.category)
def check_pairs_daily(dataframe,pair):
    """
...
# market_duo = [pair[0][0:pair[0].find("-")]+"-"+pair[1][0:pair[1].find("-")] for pair in between_chain_pairs_list]
# count_duo = Counter(market_duo)
...
for category in category_select:
    ...
    for pair in between_chain_pairs_list:
        # start = time.time()
            ...
            if os.path.exists(directory +"\\"+ pair[0] + "\\" + category + ".pkl") and os.path.exists(directory +"\\"+ pair[1] + "\\" + category + ".pkl"): #Check if these pair exists in this date
                ...
                if len(pair_prices) != 0:
                    ...
                    if len(iterating_df_of_a_pair) == 0:    
                        iterating_df_of_a_pair = check_pairs_daily(pair_prices, pair)
                        iterating_df_of_a_pair["Tot_days"] = 1 
                    else:    
                        temp_df = check_pairs_daily(pair_prices, pair)
        ...
        if len(category_df) == 0:
            category_df = iterating_df_of_a_pair.copy()
            category_df["pair_count"] = 1
        else:
            category_df = pd.merge(category_df, iterating_df_of_a_pair,how= "outer",on="Code")
            
            
            category_df["pair_count"].fillna(0,inplace=(True))
            category_df["pair_count"] += 1
            
            category_df.loc[category_df['share_of_identical_price_y'].isnull(), 'pair_count'] -= 1
            
            check_for_nan = category_df['Names_x'].isnull()
            category_df.loc[category_df['Names_x'].isnull(),"Names_x"] = category_df["Names_y"].loc[check_for_nan] 
            
            check_for_nan = category_df['Names_y'].isnull()
            category_df.loc[category_df['Names_y'].isnull(),"Names_y"] = category_df["Names_x"].loc[check_for_nan] 
            
            category_df.fillna(0,inplace=True)
            category_df["share_of_identical_price"] = category_df["share_of_identical_price_x"] + category_df["share_of_identical_price_y"]
            
            category_df.drop(labels = ["share_of_identical_price_x","share_of_identical_price_y","Names_y"],axis = 1,inplace=True)
            category_df.rename(columns = {"Names_x":"Names"},inplace = True)
    ...
    try:
        # category_df = category_df[category_df['pair_count'] >= len(list_of_pairs)*0.8] # uncomment to filter pair count
        category_df["Share_of_identical_price"] = category_df["share_of_identical_price"] / category_df["pair_count"]
        # category_df[["Names","Code","Share_of_identical_price"]].to_excel(output_directory+"//"+category +"_"+"between_chain_identical_prices.xlsx")
